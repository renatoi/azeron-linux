...
// If setup fails (permissions, etc.), the original synchronous write is preserved.
{
  ...
  // The search string here matches the post-patch-12 form. The async writer is
...
// Replace with a /proc walker that:
//   * For native processes, prints the basename of /proc/PID/exe.
//   * For Wine/Proton processes, /proc/PID/exe resolves to wine64-preloader
//     for every game (issue #25), so instead extract the last `*.exe` token
//     from /proc/PID/cmdline. That recovers names like `enshrouded.exe`,
//     which then substring-match cleanly against `ps -eo args=` in the
//     monitoring loop (Patch 16).
// `[^/\\ ]+\.exe` excludes `/`, `\`, and space inside the character class so
// the regex grabs only the trailing component of a Windows path.
{
  const listProcsScript = String.raw`for p in /proc/[0-9]*;do exe=$(readlink "$p/exe" 2>/dev/null)||continue;b=$(basename "$exe");case "$b" in wine64-preloader|wine-preloader|wineserver|wine|wine64) g=$(tr "\0" "\n" <"$p/cmdline" 2>/dev/null|grep -oiE '[^/\\ ]+\.exe'|tail -1);[ -n "$g" ] && echo "$g" || echo "$b";;*) echo "$b";;esac;done 2>/dev/null`;
  const cmdLiteral = JSON.stringify(listProcsScript);
  patch(
